...
| Tests               | 781 passed; 3 provider/live tests intentionally skipped                    |
...
- Rebuilt Sales Growth around one visible Signal → Research → Person → Outreach → Pipeline → Learn loop; added a zero-credit Apollo People Search surface and a durable, one-shot People Match allowance with every optional paid field disabled.
- Simplified the staff and CRM navigation, removed the misleading dashboard index, hid known synthetic Chat records without deleting audit data, and removed production access to record-generating demo controls.
- Removed build-time Google font downloads so a production build is reproducible without an external font fetch.
...
- `pnpm test`: 781 passed, 3 skipped, zero failed across 154 test files.
...
| [FAILURES.md](./FAILURES.md)                 | Retained build, browser, and database-preflight failure evidence                            |
