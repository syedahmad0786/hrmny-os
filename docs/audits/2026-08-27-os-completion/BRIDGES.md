...
## 15. Legacy Supabase journal → current 0068–0074 repository band

| Contract field           | Evidence-backed design                                                                                                                                                                                                                                                             |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Source side              | Canonical Supabase project `klrugedztqxlvyghyzxs`, database `postgres`, Supavisor session pooler on 5432; exactly 70 ordered Drizzle rows, legacy head `1785182400000`, fingerprint `cc60d88830c9f5d749ac6f3136ee769bf9275652b47bd8cc27b87e0651b3affd`.                            |
| Reconciled legacy schema | `crm_quote`, inbound lead-source lane, `client_onboarding`, and the historical M1 readiness columns/index must all exist before and after the change. These prove the legacy renamed tail semantically; they do not authorize journal edits.                                       |
| Destination side         | Current repository entries 0068–0074 with timestamps strictly greater than the legacy head. Expected tables cover OS modules, seam outbox, portal tokens/grants, Sales OS, policy reopening, integration inbox, and invoice gate metadata.                                         |
| Adapter                  | Drizzle timestamp selection appends seven current entries. The guard derives SHA-256 from the exact checked-out SQL bytes, requires the exact legacy prefix before writing, and after writing requires 77 rows, the unchanged first 70, and the exact seven-row tail fingerprint.  |
| Safety                   | No `DELETE`, `DROP`, down migration, journal rewrite, or secret log. Exact canonical project/database/port, DPAPI recovery receipt, confirmation phrase, GitHub Production environment, and reviewer are mandatory.                                                                |
| Acceptance               | Workflow preflight success; migration success; post-readback of journal prefix/tail, 17 secured/RLS/browser-blocked bridge tables, Sales OS columns, non-disabled Work policy, inbox uniqueness, and eight invoice columns; then `/api/ready` must report `integrationInbox=true`. |
| Recovery                 | Restore the pre-change custom-format dump plus Storage archive or apply a reviewed forward fix. Never drop receipt tables automatically or delete migration rows to retry.                                                                                                         |
