...
Goals, sub-goals, weighted contributing projects/tasks, portfolio progress and health, structured status updates, live and saved reporting dashboards, CSV export, weekly Workload, capacity allocations, project budgets/cost forecasts, task estimates, manual time entries, and single-user timers are available in beta. They operate on the shared work graph and enforce work-object permissions. Project dashboards have live bar, donut, and numeric charts; group by completion, assignee, priority, section, or task type; measure task count, estimates, or actual time; filter by completion, due range, and subtasks; and can save, reopen, or remove a view. Workload can aggregate weekly capacity, allocations, scheduled estimates, and actual time across every visible, Feature Lab-enabled project in a portfolio without double-counting multi-homed tasks. Universal cross-object reporting, custom-field chart axes, dashboard sharing, goal history, and multi-rate cost models remain beta acceptance gaps under their corresponding reporting, goals, and budgets switches.
