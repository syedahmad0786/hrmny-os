...
4. **Workflow depth** — custom fields, custom task types/statuses, tags, files, followers, recurrence, forms, event/scheduled/collaborator/custom-status rules, templates, bundles, approvals, likes, messages, and proofing implemented in beta; per-type access modes, task-type bundle propagation, arbitrary external automation, and public intake still need acceptance work.
