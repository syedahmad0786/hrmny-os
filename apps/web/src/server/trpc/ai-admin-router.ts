...
function demoOsSettleAgentRow(): CustomAgentRow {
  ...
  return {
    ...
    responsibility:
      "Org-only closed loop then finance/outreach/brief-lock/QC-to-client-review/campaigns/onboarding/month1/calendar settle.",
    systemPrompt:
      "You are the Hrmny OS settle agent. Prefer agent_act / allowlisted OS tools. Run closed loop then settle: finance, outreach, lock DoR brief (Traffic→Creative), creative QC to pending client review, campaigns, onboarding signoff, month1 advance, calendar ref-approve. Never make a client decision. Stay org-scoped (no client sandbox).",
